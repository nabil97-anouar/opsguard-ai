...
[providers](../backend/app/agent/providers) defines the typed interface, request/result contracts, context builder, deterministic implementation, OpenAI-compatible Chat Completions, OpenAI Responses, Claude Messages, and Ollama chat implementations, error mapping, and configuration factory. The deterministic implementation preserves the existing local rules in [deterministic_rules.py](../backend/app/agent/deterministic_rules.py) behind `DeterministicProvider`. It remains the default and the mandatory CI/harness provider.

Provider context contains the current alert, run-scoped evidence snapshots and IDs, trust/status metadata, suspicious observations, evidence/target gaps, prior hypotheses, blocked action definitions, and the allowed action vocabulary. It contains no database objects, API credentials, handler implementations, or unrelated history. External output is validated against Pydantic schemas and cannot directly invoke tools. Each run records provider, model, local/external mode, implementation/schema version, provider-call duration, safe response IDs, and optional token counts.

Deterministic confidence is an arithmetic function of classification, evidence count, suspicious items, and missing evidence. External-model confidence is model self-assessment. Both are constrained to `[0,1]` and are not calibrated probabilities. Application-level gaps and watchdog checks remain authoritative. Assessment decisions such as `retrieve_more` and `stop_and_request_human_review` are recorded labels; they do not change node routing.

## Evidence identity and outcomes

`EvidenceItem` validates each new observation's source type and identity. It records a run-scoped `evidence_id`, alert/document/chunk/tool-call IDs as applicable, retrieval score, trust label, content snapshot, and timezone-aware `observed_at`. Retrieval content is the exact excerpt supplied to the agent; tool content is the structured output of that particular call. For scenario tools, `observed_at` records invocation time; original infrastructure timestamps remain inside their output. Imported-observation evidence preserves a supplied source timestamp with `timestamp_basis: source_observation`. If that time is unknown, the source content retains null and evidence identifies its actual recording time with `timestamp_basis: recorded`. Bundle/observation identity and the audited tool-call ID remain exact. The model rejects quarantined supporting evidence.
...
Evidence and policy checks remain deterministic engineering controls. Deterministic hypotheses use local rules; claim-level semantic support is not verified, and injection screening covers configured patterns. Historical snapshots preserve the recorded trust and content even after a source changes; they are ordinary SQL records, not tamper-evident storage. Older records without complete provenance are not backfilled. These limits are described in [Retrieval and Evidence](RAG_DESIGN.md) and [Security Boundaries](SECURITY_BOUNDARIES.md).

Prohibited tool definitions remain context, not evidence that a model proposed them. The watchdog evaluates actual structured proposals, next steps, and planned requests for disruptive intent. A safe proposal can still require human review; an absence of destructive intent never authorizes automatic execution.

## Imported incident path

The same ten nodes process imported incidents, with execution kind `incident_investigation`. Ingestion captures the immutable bundle snapshot. Retrieval records that global document lookup is intentionally skipped; tool planning selects only the saved observation reader, one audited call per observation. This path does not query fixture metrics, jobs, networks, logs, or prior incidents. The deterministic provider uses conservative import-specific rules instead of seeded incident narratives. Schema details and limits are in [Incident Bundles](INCIDENT_BUNDLES.md).

Per-run Markdown/JSON exports render persisted step, evidence, recommendation, and audit records. They never trigger reasoning, document retrieval, or operational tools.
