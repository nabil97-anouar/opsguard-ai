...
[providers](../backend/app/agent/providers) defines the typed interface, request/result contracts, context builder, deterministic implementation, OpenAI Responses, institutional Chat Completions, Claude Messages, and Ollama chat implementations, error mapping, and configuration factory. The deterministic implementation preserves the existing local rules in [deterministic_rules.py](../backend/app/agent/deterministic_rules.py) behind `DeterministicProvider`. It remains the default and the mandatory CI/harness provider.
