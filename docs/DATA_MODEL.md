...
| `agent_runs` / `AgentRun` | `alert_id`, status, provider/model/mode/implementation/schema labels, safe provider response IDs, provider duration and optional token counts, provenance, execution kind, counters, grounding heuristic, risk, approval state, timing, error | Executed workflows and separate harness-component audit context; also labeled seeded history |
...
Tool audit storage adds `tool_execution_audits` without rewriting history, plus nullable `ToolCall.handler_invoked`, defaulted `outcome`/`origin`, and ticket lifecycle/policy fields. Existing calls receive unknown invocation (`null`) and `legacy_unknown` outcomes; old tickets receive `legacy_unknown`/`not_evaluated` and no policy version. Initialization is additive and idempotent. It does not invent executed audit checkpoints or backfill policy approval. Existing evaluation report snapshots remain unchanged.
...
## Incident bundles and investigation exports

Incident intake creates an `Alert` with a versioned, validated bundle in `raw_data`, generated bundle/observation IDs, and import time. No separate table or migration is required. Imported execution kind is `incident_investigation`; the ingest step copies the source bundle into the run before tools execute. Subsequent reads use that saved step rather than mutable alert data.

Observation evidence carries bundle/observation identity, source time when provided, untrusted status, and its audited tool-call ID. Unknown source times remain null inside the observation; canonical evidence uses the actual recording time with `timestamp_basis: recorded`. Supplied event times use `source_observation`; missing basis on legacy records is not inferred. Per-run Markdown/JSON reports render persisted steps and outcomes without fresh retrieval. Records remain ordinary SQL data that a database administrator can alter; report stability is an application behavior, not cryptographic integrity.
