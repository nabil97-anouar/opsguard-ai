...
SQLite engine setup does not enable foreign-key enforcement explicitly; constraint behavior can therefore differ from PostgreSQL. Compatibility initialization handles this milestone's additive fields, not arbitrary future schema changes. Use disposable databases for fixture reset and review relationships before deleting retained records.
